...
# default tile
DEFAULT_TITLE = 'TestRunner Test Report'
...
class HTMLTestRunner(object):
    ...
    def run(self, testlist, rerun=0, save_last_run=False):
        ...
        _, statistics = self.get_report_attributes(result)
    ...
    def get_report_attributes(self, result):
        ...
        durations = (self.end_time - self.start_time).total_seconds()
        ...
        if count > 0:
            ...
            t_percent = '{:.2%}'.format(RunResult.timeout / count)
            o_percent = '{:.2%}'.format((count-RunResult.skipped) / count)
        ...
        RunResult.timeout_rate = t_percent
        ...
        statistics_info = {
            ...
            "t": {
                "number": RunResult.timeout,
                "percent": t_percent
            },
        }
        resultData = {'testName': RunResult.title,
                      'testAll': RunResult.count,
                      'testPass': RunResult.passed,
                      'testFail': RunResult.failed,
                      'testTimeout': RunResult.timeout,
                      'testError': RunResult.errors,
                      'testSkip': RunResult.skipped,
    ...
    def _generate_heading(self, base, statistics):
        ...
        heading = env.get_template(heading_html).render(
            ...
            o_number = statistics["o"]["number"],
            o_percent = statistics['o']["percent"],
            ...
            t_number=statistics["t"]["number"],
            t_percent=statistics['t']["percent"]
        )
        return heading
    ...
    def _generate_report(self, result):
        ...
        report = env.get_template(report_html).render(
            test_list=''.join(rows),
            count=str(result.success_count + result.failure_count + result.error_count + result.skip_count + result.timeout_count),
            Pass=str(result.success_count),
            fail=str(result.failure_count),
            error=str(result.error_count),
            skip=str(result.skip_count),
            timeout=str(result.timeout_count),
