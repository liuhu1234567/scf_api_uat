...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTest(self, test) -> None:
        ...
        #GlobalTest._init()
        # restime=str(GlobalTest.get_value("restimeflag"))

        restime_here = str(customize_dict['restime_now'])
        print(f"the new restime is: {restime_here}")
        self.end_time = restime_here + "s"
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename: str = None, report_dir='.', log_path=None, theme='theme_default'):
        ...
        # self.report_dir = os.path.abspath(report_dir)+"\\reports\\"
    ...
    def output_report(self, theme):
        ...
        # 将结果传出去
        customize_dict['resultData'] = self.fields
        ...
        with open(template_path, 'rb') as file:
            ...
            #print("the soure template_path:%s" %template_path)
            body = file.read().decode('utf-8')
