...
def api_enterprise_queryInfoByName(token, payload):
    """根据名字查询企业信息"""
    url = f'{api_host}/api-scf/enterprise/queryInfoByName'
    headers = {
        "Content-Type": "application/json;charset=UTF-8",
        "x-appid-header": "2",
        "Authorization": token
...
class Enterprise(unittest.TestCase):
    ...
    def test_024_enterprise_queryInfoByName(self):
        """根据名字查询企业信息 V2.1.1新增"""
        payload = {
            "coreEntName": "",
            "projectId": ""
        }
        r = api_enterprise_queryInfoByName(token_scf_supplier, payload)
        r_json = r.json()
        restime_now = r.elapsed.total_seconds()
        customize_dict['restime_now'] = restime_now
        self.assertEqual(200, r_json['resp_code'])
