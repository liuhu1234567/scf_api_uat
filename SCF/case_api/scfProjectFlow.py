...
def api_scfProjectFlow_customerType(token):
    """【平台方】查询流程子节点名称"""
    url = f'{api_host}/api-scf/scfProjectFlow/customerType'
    headers = {
        "Content-Type": "application/json;charset=UTF-8",
        "x-appid-header": "1",
        "Authorization": token
...
def api_scfProjectFlow_formData(token):
    """【平台方】查询表单数据下拉框"""
    url = f'{api_host}/api-scf/scfProjectFlow/formData'
    headers = {
        "Content-Type": "application/json;charset=UTF-8",
        "x-appid-header": "1",
        "Authorization": token
...
class ScfProjectFlow(unittest.TestCase):
    ...
    def test_008_scfProjectFlow_scustomerType(self):
            """【平台方】查询流程子节点名称"""
            r = api_scfProjectFlow_customerType(token_scf_platform)
            r_json = r.json()
            restime_now = r.elapsed.total_seconds()
            customize_dict['restime_now'] = restime_now
            self.assertEqual(200, r_json['resp_code'])
    ...
    def test_009_scfProjectFlow_formData(self):
            """【平台方】查询表单数据下拉框"""
            r = api_scfProjectFlow_formData(token_scf_platform)
            r_json = r.json()
            restime_now = r.elapsed.total_seconds()
            customize_dict['restime_now'] = restime_now
            self.assertEqual(200, r_json['resp_code'])
