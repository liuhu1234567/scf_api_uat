...
from common.get_token import token_scf_enterprise
...
def api_scfAllocateSubsidiaries_delete(token, payload):
    """删除"""
    url = f'{api_host}/api-scf/scfAllocateSubsidiaries/delete'
    headers = {
        "Content-Type": "application/json;charset=UTF-8",
        "x-appid-header": "2",
        "Authorization": token
...
class ScfAllocateSubsidiaries(unittest.TestCase):
    ...
    def test_007_scfAllocateSubsidiaries_delete(self):
        """【核心企业】删除"""
        payload = {
            "id": 0
        }
        r = api_scfAllocateSubsidiaries_delete(token_scf_enterprise, payload)
        r_json = r.json()
        restime_now = r.elapsed.total_seconds()
        customize_dict['restime_now'] = restime_now
        self.assertEqual(200, r_json['resp_code'])
