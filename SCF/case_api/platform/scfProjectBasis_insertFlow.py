...
def api_scfProjectBasis_insertFlow(token, payload):
    """【平台方】准入配置-创建流程"""
    url = f'{api_host}/api-scf/scfProjectBasis/insertFlow'
    headers = {
...
class ScfProjectBasisInsertFlow(unittest.TestCase):
    def test_scfProjectBasis_insertFlow(self):
        """【平台方】准入配置-创建流程"""
        contact = get_name()
        entName = get_company()
